...
namespace ConsoleUnitTesting
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            //TEST Get All Producttype with quantity 
            ProducttypeDB db = new ProducttypeDB();
            List<Producttype> list = await db.GetAllWithQuantityAsync();
            for (int i = 0; i < list.Count; i++)
            {
                await Console.Out.WriteLineAsync(@$" id={list[i].TypeID} name={list[i].TypeName} Quantity={list[i].QuantityInCategory}");
            }
            await Console.Out.WriteLineAsync("\n\n");
